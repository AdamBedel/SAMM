    ...
    def run_with_diffusion_eta(self):
        ...
        for n in range(Nt - 1):
            ...
            if n > 0:
                ...
                _, _, phi = physics.diffusion_nonuniform_eta(
                    x_liner, self.B, dt, T_profile, eos.eta_of_T,
                    right=rightBC, C=0.99
                )
                self.B = phi[-1]   # final profile, shape (N,)



            btheta_iface = np.zeros(self.B.size + 1)  # (N_SHELLS+1,)
            btheta_iface[1:] = self.B
            btheta_iface[0]  = btheta_iface[1]
